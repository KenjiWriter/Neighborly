...
const form = useForm({
    ...
    audience_type: props.announcement.audience_type,
    roles_selected: props.targeted_roles,
    buildings_selected: props.targeted_buildings,
    units_selected: [] as number[], // Simplification
    published: !!props.announcement.published_at,
});

const submit = () => {
    form.put(routes.announcementsUpdate(props.announcement.id));
};
</script>

<template>
    <Head :title="t('announcements.edit_title')" />

    <AppLayout>
        <PageHeader :title="t('announcements.edit_title')" />
